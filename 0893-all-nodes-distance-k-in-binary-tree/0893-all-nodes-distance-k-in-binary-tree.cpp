...
public:
    ...
    //Lets make the tree undirected, and to do so we can take a map to store the node and the parent, by doing inorder traversal
    void addParent(TreeNode* root) {
        if(!root)
            return;
        
        if(root->left)
            parent[root->left] = root; //before going to the child, make the node as parent of its child
